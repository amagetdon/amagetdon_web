...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { path: '/admin/revenue-analytics', label: '매출 통계', icon: 'ti-report-money' },
    ],
  },
  {
    label: '회원',
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/admin/performance', label: '성능 모니터', icon: 'ti-activity' },
    ],
  },
]
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-120px)]">
      {/* Desktop sidebar */}
      <aside className="w-[240px] bg-[#1c1c1c] shrink-0 flex flex-col max-lg:hidden">
        {sidebar}
      </aside>
      ...
      <button
        ...
        className="hidden max-lg:fixed max-lg:block top-[76px] left-4 z-40 w-10 h-10 bg-[#1c1c1c] rounded-lg flex items-center justify-center border-none cursor-pointer shadow-lg"
        aria-label="메뉴"
      >
      ...
      {mobileOpen && (
        <>
          ...
          <aside className="fixed left-0 top-0 bottom-0 w-[240px] bg-[#1c1c1c] z-40 flex flex-col lg:hidden">
            {sidebar}
          </aside>
        </>
      )}
