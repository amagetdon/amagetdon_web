    ...
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setEditing({ question: '', answer: '', is_published: true })}
          className="bg-[#2ED573] text-white px-4 py-2 rounded-xl text-sm font-bold cursor-pointer border-none hover:bg-[#25B866] transition-colors shadow-sm shadow-[#2ED573]/20 flex items-center gap-1.5"><i className="ti ti-plus text-sm" /> FAQ 추가</button>
      </div>
        ...
        {editing && (
          <div className="flex flex-col gap-4">
            ...
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={editing.is_published ?? true} onChange={(e) => setEditing({ ...editing, is_published: e.target.checked })} className="accent-[#2ED573]" /> 공개
            </label>
          </div>
        )}
      </AdminFormModal>
