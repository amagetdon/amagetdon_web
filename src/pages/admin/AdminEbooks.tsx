              ...
              {filtered.length === 0 ? (
                ...
                <tr key={eb.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <button onClick={() => setEditing(eb as unknown as Record<string, unknown>)} className="text-blue-500 text-xs cursor-pointer bg-transparent border-none mr-2 hover:underline">수정</button>
                    <button onClick={() => setDeleteTarget(eb.id)} className="text-red-500 text-xs cursor-pointer bg-transparent border-none hover:underline">삭제</button>
                  </td>
                </tr>
              ))}
            </tbody>
